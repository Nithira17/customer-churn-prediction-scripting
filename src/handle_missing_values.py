...
class DropMissingValuesStrategy(MissingValueHandlingStrategy):
    ...
    def handle(self, df):
        ...
        logging.info(f'{n_dropped} rows has been dropped')
        return df_cleaned
...
class GenderImputer:
    ...
    def _predict_gender(self, firstname, lastname):
        prompt = f"""
                    What is the most likely gender (Male or Female) for someone with the frist name '{firstname}' and last name '{lastname}'?
...
class FillMissingValuesStrategy(MissingValueHandlingStrategy):
    ...
    def __init__(self, method='mean', fill_value=None, relevant_column=None, is_custom_imputer=False, custom_imputer=None):
        self.method = method
